...
3. booking-service instantly sends request (through kafka) for payment to -> payment-service and -> catalog service to reserve it , it sends back to necessary topic and booking-service sends request to catalog to take (change status SOLD)
...
Each service is tested in isolation, and analytics-service is used to validate event flow correctness
